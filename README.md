...
щільніший за ці макети, але секції та їхній зміст ті самі):
...
- Використана ємність сьогодні/тиждень/місяць/всього — та кнопка в
  редакторі картки, яка сама створює потрібні helper-сенсори (див. нижче)
...
картки (клацнути на картку → "Show code editor" вимкнено, а внизу форми,
під основними полями) з'явиться кнопка **"Створити сенсори споживання і часу
роботи"**. Вона:
...
2. Створює один helper `integration` (Riemann sum, накопичена ємність за
   весь час — не скидається) на основі `entities.power`.
3. Створює три helpers `utility_meter` (daily/weekly/monthly) поверх щойно
   створеного integration-сенсора.
4. Автоматично підставляє отримані `entity_id` в `entities.capacity_*`
   конфіга картки.

Потрібні admin-права користувача HA; без них картка покаже повідомлення і
запропонує мануальну інструкцію вище. Майстер користується тим самим
...
версіях HA — мануальний спосіб вище завжди залишається робочим fallback.
