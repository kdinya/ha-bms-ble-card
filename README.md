...
Усі `entities.*` можна вибрати у візуальному редакторі картки — там для
кожного поля є звичний HA entity picker (пошук за назвою, автодоповнення),
поля згруповані у розділи, що розгортаються. YAML нижче — те саме, просто
у текстовому вигляді (зручно для копіювання/бекапу конфігурації):
