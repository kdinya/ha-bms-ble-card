 ...
 * оновлюється через hass). Іскристий сigment/"sweep" залишений як
 * безкінечна SMIL-анімація (просто декоративний блиск, не залежить
 * від відсотка).
 */
...
function jarBatterySvg(uid, percent, voltageLabel) {
  ...
  const shineOpacity = p > 0 ? "0.16" : "0.08";
      ...
      <svg class="jar-battery-overlay" viewBox="0 0 402 824" preserveAspectRatio="xMidYMid meet" aria-hidden="true">
        <defs>
          ...
          <linearGradient id="${id("sweep")}" x1="0" y1="0" x2="1" y2="0">
            ...
            <stop offset=".50" stop-color="white" stop-opacity=".40"/>
        ...
        <g clip-path="url(#${id("innerBatteryClip")})" pointer-events="none">
          ${liquidMarkup}
          <rect x="-140" y="150" width="70" height="600" fill="url(#${id("sweep")})" opacity="${shineOpacity}" transform="skewX(-8)">
            <animate attributeName="x" from="-170" to="450" dur="4.8s" repeatCount="indefinite"/>
          </rect>
        </g>
